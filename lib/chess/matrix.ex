defmodule Chess.Matrix do
    ...
    |> Enum.reduce(matrix, fn column, acc_matrix ->
      ...
      |> Enum.reduce(acc_matrix, fn line, acc ->
        ...
        %{acc |
          ...
          anti_diagonals: create_or_update_keyword(acc.anti_diagonals, anti_diagonal_key, position),
  ...
  defp build_key(prefix, value) when is_binary(value), do: String.to_atom("#{prefix}#{String.upcase(value)}")
  defp build_key(prefix, value) when is_integer(value) and value < 0, do: String.to_atom("#{prefix}M#{abs(value)}")
